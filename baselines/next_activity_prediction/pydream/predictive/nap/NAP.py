...
class NAP:
    ...
    def train(self,
        ...
        early_stopping = EarlyStopping(
            ...
            patience=32,
            verbose=1,
            restore_best_weights=True  # automatically restore best model
        )
    ...
    def predict_bulk(self, tss: list):
        """
        Predict from a list of TimedStateSamples in bulk.
    
        :param tss: list<TimedStateSample>
        :return: tuple (DREAM-NAP output as ints, translated next events as strings)
        """
        if not isinstance(tss, list) or not isinstance(tss[0], TimedStateSample):
            raise ValueError("Input is not a list with TimedStateSample")
    
        # Extract features and true values for all samples
        features_list, y_true_label = [], []
        for sample in tss:
            if sample.export()["label"] is not None:
                features_list.append(list(itertools.chain(
                    sample.export()["tss"][0],
                    sample.export()["tss"][1],
                    sample.export()["tss"][2]
                )))
                y_true_label.append(sample.export()["label"])
    
        # Transform features in bulk
        features_array = self.stdScaler.transform(features_list)
    
        # Predict in bulk (integers)
        preds = np.argmax(self.model.predict(features_array), axis=1)
    
        # Convert integers to event strings
        next_events = [self.__intToEvent(p) for p in preds]
    
        # Return both integer predictions and string translations
        return list(preds), next_events, y_true_label
