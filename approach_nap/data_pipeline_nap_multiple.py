...
def trace_to_multiple_graphs(trace, activity_to_idx,
    ...
    divisor, divisor2 : float  — time normalization from train+val
    ...
    for t in range(1, len(sorted_times)):
        ...
        x, edge_index, edge_attr, node_pos = _build_prefix_graph(
            prefix_events, activity_to_idx, timestamp, divisor, divisor2)

        # Time from the last prefix block to the target block
        last_prefix_ts = sorted_times[t - 1]
        dt = (target_ts - last_prefix_ts).total_seconds() / divisor
        y_time = torch.tensor([dt], dtype=torch.float)
