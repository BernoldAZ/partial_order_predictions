"""Train and evaluate GATv2EncoderGRUDecoderSuffixStop for activity suffix prediction
only (no time prediction). Identical to run_suffix_v1.py but adds a binary stop-head
loss that explicitly trains the model to predict end-of-sequence.

Usage
-----
    python run_suffix_v2.py <log_name> [results_dir]
...
from model_suffix_v2 import GATv2EncoderGRUDecoderSuffixStop
...
def _loss(act_logits, stop_logits, data, num_activities, avg_suffix_len):
...
def run(log_name: str, results_dir: str = None):
    ...
    results_dir = results_dir or f'results_suffix_{METHOD_NAME}'
    ...
    # ── Build model ───────────────────────────────────────────────────────────
    model = GATv2EncoderGRUDecoderSuffixStop(
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for data in train_loader:
            data = data.to(device)
            optimizer.zero_grad()
            act_logits, stop_logits = model(data, p_teacher=p_teacher)
            loss = _loss(act_logits, stop_logits, data, num_activities, avg_suffix_len)
            loss.backward()
...
def _parse_args():
    p = argparse.ArgumentParser(
        description='Train GRU+stop-head model and evaluate for suffix-only prediction')
