...
from Preprocessing.from_log_to_tensors_graph import log_to_graphs
...
def construct_datasets(
    ...
    train_data, val_data, test_data, counts = log_to_graphs(
        log,
        log_name=log_name,
        start_date=start_date,
        start_before_date=start_before_date,
    ...
    torch.save(train_data, os.path.join(output_directory, 'train_graphdataset.pt'))
    torch.save(val_data,   os.path.join(output_directory, 'val_graphdataset.pt'))
    torch.save(test_data,  os.path.join(output_directory, 'test_graphdataset.pt'))
