"""Train and evaluate GATv2EncoderGRUDecoderNewBlockV2 for activity
suffix + TTNE prediction.  Same architecture as v1; the only difference
is that training uses teacher forcing only (no scheduled sampling).

Usage
-----
    python run_suffix_time_v2.py <log_name> [results_dir]
...
from model_suffix_time_v2 import GATv2EncoderGRUDecoderNewBlockV2
...
# ─── Scheduled sampling ───────────────────────────────────────────────────────
USE_SCHEDULED_SAMPLING = False   # v2 trains with teacher forcing only

# ─── New-block loss weight ────────────────────────────────────────────────────
NB_WEIGHT = 1.0   # scalar multiplier on the new-block BCE loss term

METHOD_NAME = 'gatv2_gru_nb_v2'
GES_WORKERS = 4
...
def run(log_name: str, results_dir: str = None, run_id: int = 0):
    ...
    # ── Build model ───────────────────────────────────────────────────────────
    model = GATv2EncoderGRUDecoderNewBlockV2(
...
def _parse_args():
    p = argparse.ArgumentParser(
        description='Train GRU+new-block v2 model (teacher forcing only) and evaluate for suffix + RRT')
