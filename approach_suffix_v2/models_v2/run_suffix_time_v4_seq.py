"""Train and evaluate GATv2EncoderGRUDecoderStopNewBlock (v4) for activity
suffix + TTNE prediction.  v4 is identical in behaviour to v3 but is a
fully standalone class (no inheritance from v1/v2).

Usage
-----
    python run_suffix_time_v4_seq.py <log_name> [results_dir]
...
from model_suffix_time_v4 import GATv2EncoderGRUDecoderStopNewBlock
...
METHOD_NAME = 'gatv2_seq_gru_stop_nb_v4'
GES_WORKERS = 4
...
def _loss(act_logits, ttne_preds, stop_logits, nb_logits,
          data, num_activities, avg_suffix_len, nb_pos_weight):
    ...
    return ce + mae + stop + NB_WEIGHT * nb
...
def run(log_name: str, results_dir: str = None):
    ...
    # Average non-padding suffix length (incl. END); used as pos_weight for stop BCE.
    avg_suffix_len = sum(
    ...
    print(f"num_activities={num_activities}  window_size={window_size}  "
          f"avg_suffix_len={avg_suffix_len:.1f}  nb_pos_weight={nb_pos_weight:.2f}")

    train_loader = DataLoader(train_data, batch_size=BATCH_SIZE, shuffle=True,  drop_last=True)
    val_loader   = DataLoader(val_data,   batch_size=BATCH_SIZE, shuffle=False)

    # ── Build model ───────────────────────────────────────────────────────────
    model = GATv2EncoderGRUDecoderStopNewBlock(
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for data in train_loader:
            data = data.to(device)
            optimizer.zero_grad()
            act_logits, ttne_preds, stop_logits, nb_logits = model(
                data,
                p_teacher=p_teacher,
            ...
            loss = _loss(act_logits, ttne_preds, stop_logits, nb_logits,
                         data, num_activities, avg_suffix_len, nb_pos_weight)
            loss.backward()
...
def _parse_args():
    p = argparse.ArgumentParser(
        description='Train GRU+stop+new-block v4 model (seq graph data) and evaluate for suffix + RRT')
