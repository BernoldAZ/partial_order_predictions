"""
GATv2 encoder + GRU decoder with stop head and new-block head.
Standalone implementation — no inheritance from v1/v2/v3.

Predicts:
  - activity suffix      (fc_out_act)
  - time to next event   (fc_out_ttne)
  - end-of-trace signal  (fc_stop)    BCE-trained; soft-biased into argmax at inference
...
class GATv2EncoderGRUDecoderStopNewBlock(nn.Module):
    """
    Standalone GATv2 encoder + GRU decoder with stop and new-block heads.
    ...
    def _teacher_forcing(self, data, h0):
        ...
        # t=0 uses last prefix event's tss/tsp; t>0 uses suffix ground truth shifted by 1
        ...
        return self.fc_out_act(output), self.fc_out_ttne(output), stop_logits, nb_logits
    ...
    def _scheduled_sampling(self, data, h0, p_teacher, mean_std_ttne, mean_std_tss, mean_std_tsp):
        ...
        all_act_logits, all_ttne, all_stop, all_nb = [], [], [], []
        ...
        return act_out, ttne_out, stop_out, nb_out
    ...
    def _autoregressive(self, data, h0, window_size, mean_std_ttne, mean_std_tss, mean_std_tsp):
        ...
        for t in range(W):
            ...
            act_logits[:, 0]  = -1e9
            act_logits[:, -1] = act_logits[:, -1] + stop_logit
