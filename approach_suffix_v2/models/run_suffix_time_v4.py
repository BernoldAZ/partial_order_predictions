"""Train and evaluate GATv2CrossAttnGRUDecoder for activity suffix + TTNE
prediction. Extends v2 (stop head + scheduled sampling) by replacing the
single compressed h0 context with per-step cross-attention over all encoded
prefix nodes.
...
from model_suffix_time_v4 import GATv2CrossAttnGRUDecoder
...
METHOD_NAME = 'gatv2_cross_attn_gru'
...
def run(log_name: str, results_dir: str = None):
    ...
    # ── Build model ───────────────────────────────────────────────────────────
    model = GATv2CrossAttnGRUDecoder(
...
def _parse_args():
    p = argparse.ArgumentParser(
        description='Train cross-attention GRU+stop-head model and evaluate for suffix + RRT')
