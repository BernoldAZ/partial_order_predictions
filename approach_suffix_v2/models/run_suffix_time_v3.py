"""Train and evaluate GATv2DataAwareEncoderGRUDecoderStop for activity suffix
+ TTNE prediction. Extends v2 by feeding all prefix node features (numeric
and non-activity categorical) into the encoder, not just the activity label.
...
from model_suffix_time_v3 import GATv2DataAwareEncoderGRUDecoderStop
...
METHOD_NAME = 'gatv2_data_aware_gru_stop'
...
def run(log_name: str, results_dir: str = None):
    ...
    cat_cardinalities = pref_cat_cars[:-1]
    n_numeric         = train_data[0].x.shape[1]
    ...
    print(f"num_activities={num_activities}  window_size={window_size}  "
          ...
          f"cat_features={len(cat_cardinalities)}  numeric_features={n_numeric}")

    train_loader = DataLoader(train_data, batch_size=BATCH_SIZE, shuffle=True, drop_last=True)
    val_loader   = DataLoader(val_data,   batch_size=BATCH_SIZE, shuffle=False)

    # ── Build model ───────────────────────────────────────────────────────────
    model = GATv2DataAwareEncoderGRUDecoderStop(
    ...
    # ── Training loop (validation uses greedy for speed) ──────────────────────
...
def _parse_args():
    p = argparse.ArgumentParser(
        description='Train data-aware GRU+stop-head model and evaluate for suffix + RRT')
