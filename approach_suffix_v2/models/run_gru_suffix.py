"""Train and evaluate GRUEncoderGRUDecoder for activity-suffix, time, and
concurrency prediction.

Usage
-----
    python approach_suffix_v2/models/run_gru_suffix.py <log_name> [results_dir]
...
from model_gru_suffix import GRUEncoderGRUDecoder
...
def _loss(act_logits, time_pred, conc_logits, data, num_classes):
    ce = F.cross_entropy(
...
def run(log_name: str, results_dir: str = None):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for batch in train_loader:
            batch = _to_device(batch, device)
            optimizer.zero_grad()
            act_logits, time_pred, conc_logits = model(batch, p_teacher=p_teacher)
            loss = _loss(act_logits, time_pred, conc_logits, batch, num_classes)
            loss.backward()
    ...
    # ── Save results ──────────────────────────────────────────────────────────
    csv_path   = os.path.join(results_dir, 'results_gru_suffix.csv')
...
def _parse_args():
    p = argparse.ArgumentParser(
        description='Train GRU encoder-decoder; evaluate with DL similarity, '
