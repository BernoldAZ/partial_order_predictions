...
prediction. Identical to run_suffix_time_v1.py (v1) but adds a binary stop-head
...
# ─── Scheduled sampling ───────────────────────────────────────────────────────
USE_SCHEDULED_SAMPLING = True    # set True to replace teacher forcing with SS
...
def _stop_bce(stop_logits, act_targets, num_activities, avg_suffix_len):
    ...
    logits  = stop_logits.view(-1)                                     # (B*W,)
    targets = act_targets.view(-1)                                     # (B*W,)
...
def _parse_args():
    p = argparse.ArgumentParser(
        description='Train GRU+stop-head model and evaluate for suffix + RRT')
