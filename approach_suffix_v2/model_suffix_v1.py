...
+ GRU decoder for activity suffix prediction only (no time prediction).
...
Inference : greedy autoregressive decoding; returns (B, W) integer activity suffix.
"""
...
class GATv2EncoderGRUDecoderSuffix(nn.Module):
    def __init__(self, num_activities: int, d_model: int = 64,
        ...
        # GNN encoder
        self.sage      = SAGEConv(emb_size, d_model)
        ...
        # GRU decoder: act_emb only (no time features)
    ...
    def _encode(self, data):
        ...
        h = self.dropout(h)
        h = self.sage(h, data.edge_index).relu()
    ...
    def _teacher_forcing(self, data, h0):
        ...
        return self.fc_out_act(output)             # (B, W, C)
    ...
    def _scheduled_sampling(self, data, h0, p_teacher):
        ...
        return torch.stack(all_act_logits, dim=1)  # (B, W, C)
