...
---------------------------------------
  suffix      → results/run_N/results_suffix_gnn.csv
  suffix_time → results_time/run_N/results_suffix_gnn_time.csv

Usage
-----
    python run_all_suffix.py                                       # suffix, run 1, 1 worker
    python run_all_suffix.py --workers 4                           # suffix, run 1, 4 logs in parallel
    python run_all_suffix.py --model suffix_time                   # suffix_time, run 1
    python run_all_suffix.py --model suffix_time --run-id 2 --workers 4
    python run_all_suffix.py --logs-dir /path/to/logs

Usage with docker:
    docker run -it --rm -v $(pwd):/workspace --gpus all ml-jupyter-gpu python approach_suffix/run_all_suffix.py --workers 1 --model suffix_time
"""
...
MODEL_CONFIGS = {
    'suffix': {
        'module':      'approach_suffix.run_suffix',
        'results_sub': 'results',
        'csv_file':    'results_suffix_gnn.csv',
    },
    'suffix_time': {
        'module':      'approach_suffix.run_suffix_time',
        'results_sub': 'results_time',
        'csv_file':    'results_suffix_time.csv',
    },
}
...
def _progress_file(model, run_id):
    return os.path.join(_HERE, f"run_all_suffix_{model}_progress_run{run_id}.log")
...
def run_all(model='suffix', run_id=1, progress_file=None, logs_dir=None, workers=1):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Which model to run: suffix, suffix_time (default: suffix)",
    )
    parser.add_argument(
        "--run-id",
        type=int,
