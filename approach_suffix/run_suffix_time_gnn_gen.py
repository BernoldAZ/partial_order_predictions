"""
Train and evaluate the GraphGenModel (GNN encoder + LSTM decoder).

Usage
-----
    python run_graph_gen.py <log_path> [log_name] [results_dir]
"""
...
from approach_suffix.model_gnn_gen import GraphGenModel, train_epoch, evaluate
...
def run(log_path, log_name, results_dir):
    ...
    print(f"Model  : GraphGen  (GNN encoder + LSTM decoder)")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        train_loss = train_epoch(
            ...
            end_token_idx, mean_ttne, LAMBDA_TIME)
    ...
    csv_path   = os.path.join(results_dir, 'results_graph_gen.csv')
    ...
    updated = False
    for row in rows:
        if row['log'] == log_name and row['model'] == 'GraphGen':
            row.update(new_row)
            updated = True
            break
    if not updated:
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description='Graph generation model: suffix activity + time prediction')
