...
def inference_loop(model, 
    ...
    # Retrieving labels — layout: [ttne, rrt, act, concurrent]
    labels_global = inference_dataset[-4:]

    # Retrieving seq length (`window_size`, also referred to as W)
    act_label_index = 2  # act is 3rd of 4 labels
    ...
    with torch.no_grad():
        ...
        for valbatch_num, vdata in tqdm(enumerate(inference_dataloader), desc="Validation batch calculation"):
                # lstm_dataset layout: [..., ttne, rrt, act, concurrent]
                vinputs = vdata[:-4]
                vlabels = vdata[-4:]
                ...
                act_labels = vlabels[2]  # act is 3rd of 4 labels
                ...
                for i in range(window_size):
                    # Model predictions: next activity, TTNE, and concurrent flag
                    voutputs = model(infer_env.inputs)
                    act_preds, ttne_preds, concurrent_preds = voutputs[0], voutputs[1], voutputs[2]

                    # If model predicts the next event is concurrent, TTNE must be 0 seconds.
                    # Zero seconds in standardized space = -mean/std, not 0.
                    concurrent_bool = concurrent_preds.sigmoid() > 0.5  # (batch_size, 1)
                    stand_zero_seconds = -mean_std_ttne[0] / mean_std_ttne[1]
                    ttne_preds = torch.where(concurrent_bool, torch.full_like(ttne_preds, stand_zero_seconds), ttne_preds)
