...
class BatchInference():
    ...
    def decode_step(self, 
        ...
        if dec_step < (self.window_size-1):
            ...
            # Update numerical prefix features
            num_ftrs_pref = self.inputs[self.num_categoricals_pref].clone() # (B, W, num_numericals_pref)

            tss_tsp = num_ftrs_pref[:, :, :2].clone() # (B, W, 2)
            extra_ftrs = num_ftrs_pref[:, :, 2:].clone() # (B, W, num_numericals_pref-2)

            # Discarding first (left-padded) padding token for tss/tsp
            ...
            # For extra features (day_sin, day_cos, hour_sin, hour_cos, concurrent):
            # shift left by 1 and pad with 0 for the new predicted event,
            # since absolute timestamps are not available in the inference loop.
            num_extra = extra_ftrs.shape[-1]
            if num_extra > 0:
                extra_ftrs_new = extra_ftrs[:, 1:, :].clone() # (B, W-1, num_extra)
                zero_pad = torch.zeros(self.batch_size, 1, num_extra, dtype=torch.float32, device=device)
                extra_ftrs_new = torch.cat((extra_ftrs_new, zero_pad), dim=1) # (B, W, num_extra)
                new_num_ftrs = torch.cat((tss_tsp_new, extra_ftrs_new), dim=-1) # (B, W, num_numericals_pref)
            else:
                new_num_ftrs = tss_tsp_new

            # Updating the numerics
            self.inputs[self.num_categoricals_pref] = new_num_ftrs # (B, W, num_numericals_pref)
