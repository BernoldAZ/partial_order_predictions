...
def convert_to_SEP_data(data, 
    ...
    * converting the originally right-padded prefix tensors into 
    ...
    # Slicing out the numeric features: tss, tsp, concurrent
    numeric_inds = [tss_index, tss_index + 1, tss_index + 6]
    _NUM_NUMERICAL = len(numeric_inds)  # 3

    numeric_inds_tens = torch.tensor(numeric_inds, dtype=torch.long)
    num_ftrs_pref = torch.index_select(num_ftrs_pref, dim=-1, index=numeric_inds_tens) # (num_prefs, W, 7)


    # Selecting the activity suffix, runtime, ttne, and concurrent labels.
    # Label order in all_tensors (from tensor_creation.py):
    #   ... ttnext_tens, rtime_tens, concurrent_tens, actLabel_tens [, outLabel_tens]
    if outcome_bool:
        act_labels          = data[-2] # (num_prefs, window_size)
        concurrent_labels   = data[-3] # (num_prefs, window_size, 1)
        rrt_labels          = data[-4] # (num_prefs, window_size, 1)
        ttne_labels         = data[-5] # (num_prefs, window_size, 1)
    else:
        act_labels          = data[-1] # (num_prefs, window_size)
        concurrent_labels   = data[-2] # (num_prefs, window_size, 1)
        rrt_labels          = data[-3] # (num_prefs, window_size, 1)
        ttne_labels         = data[-4] # (num_prefs, window_size, 1)

    # Binarize concurrent labels for the first suffix step.
    # concurrent_labels contains standardized 0/1 values; since concurrent
    # proportion is <50%, standardized-1 is positive and standardized-0 is negative.
    concurrent_labels = (concurrent_labels[:, 0, 0] > 0.0).to(torch.float32) # (num_prefs,)
    ...
    cat_inputs, num_ftrs_pref = left_pad(cat_inputs=cat_inputs,
                                         ...
                                         num_nums=_NUM_NUMERICAL)

    
    # Layout: [cat..., num_pref, pad_mask, ttne, rrt, act, concurrent]
    # Training and inference loops split as data[:-4] / data[-4:].
    lstm_dataset = cat_inputs + (num_ftrs_pref,) + (padding_mask_input,) + (ttne_labels,) + (rrt_labels,) + (act_labels,) + (concurrent_labels,)
