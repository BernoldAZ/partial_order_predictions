...
class SEP_Benchmarks_time(nn.Module):
    def __init__(self,
        ...
        self.dim_init_prefix = emb_size_acts + num_numericals_pref
        ...
        # Dense concurrent prediction layer (binary; logit, apply sigmoid externally)
        self.fc_concurrent = nn.Linear(self.d_model, 1)
        init.xavier_uniform_(self.fc_concurrent.weight)
    ...
    def forward(self, inputs):
        ...
        # Concurrent prediction from the shared LSTM last step output (logit)
        concurrent_pred = self.fc_concurrent(shared_out[:, -1, :]) # (batch_size, 1)

        return act_probs, ttne_pred, concurrent_pred