...
def create_numeric_timeCols(df, 
    ...
    # (case_length may already be set upstream, but we recompute here for safety
    # in case this function is called independently.)
    df['case_length'] = df.groupby(case_id, sort=False)[act_label].transform('count')

    # NOTE: sort_log is intentionally omitted here.  The dataframe is already
    # sorted (sort_log is called at the top of main_dataframe_pipeline, and
    # nothing downstream disturbs that order before create_numeric_timeCols
    # is called).

    # Compute first and last timestamp per case using transform (avoids two merges).
    df['first_stamp'] = df.groupby(case_id, sort=False)[timestamp].transform('first')
    df['last_stamp']  = df.groupby(case_id, sort=False)[timestamp].transform('last')
    ...
    # Concurrency flag: 1 if event i shares its timestamp with event i-1 in the same case.
    # First event of each case gets 0 (NaT comparison yields False).
    df['concurrent'] = (df[timestamp] == df['previous_stamp']).astype(int)
    ...
    # Cyclic encoding of day-of-week and hour-of-day from the event timestamp.
    # Sin/cos pairs preserve the cyclic structure (Mon≈Sun, 23h≈0h).
...
def split_train_val(df, 
    ...
    """
    # Getting df with 2 cols, case id and the start timestamp for each case.
    # groupby().min() is faster than pivot_table for this simple aggregation.
    case_start_df = (df.groupby(case_id, sort=False)[timestamp]
                       .min()
                       .reset_index()
...
def main_dataframe_pipeline(log,
    ...
    time_cycle_cols = ['day_sin', 'day_cos', 'hour_sin', 'hour_cos', 'concurrent']
    pref_numcols = num_casefts + num_eventfts + ['ts_start', 'ts_prev'] + time_cycle_cols
    suff_numcols = ['ts_start', 'ts_prev'] + time_cycle_cols
    timelab_numcols = ['tt_next', 'rtime', 'concurrent']
